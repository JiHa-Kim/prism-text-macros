...
export const defaultSnippets: Macro[] = [
  ...
  { trigger: "G", replacement: "\\textcolor{lime}{${VISUAL}}$0", options: "mA", description: "color lime" },
  { trigger: "B", replacement: "\\textcolor{blue}{${VISUAL}}$0", options: "mA", description: "color blue" },
  { trigger: "Y", replacement: "\\textcolor{yellow}{${VISUAL}}$0", options: "mA", description: "color yellow" },
  ...
  // Environments (text-mode variants removed)
  ...
  { trigger: "form", replacement: "\\left\\langle $0 \\mid $1 \\right\\rangle $2", options: "mA" },
  ...
  { trigger: /iden(\d)/, replacement: (match: any) => {
      const n = match[1];
      let arr: number[][] = [];
      for (let j = 0; j < n; j++) {
        arr[j] = [];
